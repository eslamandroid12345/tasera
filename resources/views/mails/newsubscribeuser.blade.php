...
<div style="background: linear-gradient(363deg, #f8ecff 0%, rgba(217, 217, 217, 0) 100%); padding: 0; overflow: hidden;">
    <table class="container" width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 800px; margin: 0 auto;">
        ...
        <tr>
            <td>
                <p style="text-align: right;direction: rtl;"> الشركة , {{ $data['user']->company->name_ar }} برقم مرجعى  {{ $data['user']->company->reference_id }}</p>
                <p style="text-align: right;direction: rtl;">      ترغب بالاشتراك في الباقة  :  {{ $data['package']->name_ar }}</p>
                @if($data['type'] == '1')
                    <p style="text-align: right;direction: rtl;">     عبر بيانات التواصل التاليه  : البريد الالكترونى  {{ $data['user']->email }} , رقم الجوال  {{ $data['user']->phone }}</p>
                @else
                    <p style="text-align: right;direction: rtl;">     عبر بيانات التواصل التاليه  : البريد الالكترونى  {{ $data['email'] }} , رقم الجوال  {{ $data['phone'] }}</p>
                @endif
                <p style="text-align: right;direction: rtl;">نتمنى لكم رحلة سعيدة.</p>
                <p style="text-align: right;direction: rtl;">لمراجعة تفاصيل الاشتراك تفضل بزيارة رابط تفاصيل الاشتراك <a href="https://quotation.sa">https://quotation.sa</a></p>

                <p>Dear partner, {{ $data['user']->company->name_en }}, Supplier's reference number: {{ $data['user']->company->reference_id }}</p>
                <p>wants to subscribe to the package :  {{ $data['package']->name_en }}</p>
                @if($data['type'] == '1')
                    <p>Through the following contact information :  email : {{ $data['user']->email }} , phone : {{ $data['user']->phone }}</p>
                @else
                    <p>Through the following contact information :  email : {{ $data['email'] }} , phone : {{ $data['phone'] }}</p>
                @endif
                <p>We wish you a happy trip</p>
                <p>To review the subscription details, visit the subscription details link <a href="https://quotation.sa">https://quotation.sa</a></p>
            </td>
        </tr>
        <tr>
            <td align="center">
