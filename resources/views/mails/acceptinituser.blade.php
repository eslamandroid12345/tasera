    ...
    <table class="container" width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 800px; margin: 0 auto;">
        ...
        <tr>
            <td>
                ...
                <p style="text-align: right;direction: rtl;">نرحب بك في منصة تسعيرة ونفيدك باعتماد تسجيل حسابكم كمستخدم لحساب المورد ({{ $data['user']->company->name_ar }}) في منصة تسعيرة برقم مرجعي {{ $data['user']->company->reference_id }} </p>
                ...
                <p>We welcome you to quotation platform and glad to inform you that your account registration been approved as a user for a supplier company ({{$data['user']->company->name_en}}), your account as a supplier on quotation platform with reference number {{ $data['user']->company->reference_id }}</p>
