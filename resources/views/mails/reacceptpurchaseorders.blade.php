    ...
    <table class="container" width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 800px; margin: 0 auto;">
        ...
        <tr>
            <td>
                ...
                <p style="text-align: right;direction: rtl;">   نفيدكم بانه تمت الموافقه على طلب تعديل طلب الشراء الخاص بكم بعنوان : {{ $data['title'] }}  ورقم مرجعي {{ $data['body'] }} </p>
                <p style="text-align: right;direction: rtl;"><a href="{{ $data['link'] }}">رابط طلب الشراء</a></p>
                ...
                <p>We inform you that your modification on purchase order has been approved to be published with the address: {{ $data['title'] }} and reference number {{ $data['body'] }}</p>
                <p><a href="{{ $data['link'] }}">Purchase order link</a></p>
