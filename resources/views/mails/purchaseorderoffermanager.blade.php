...
{{--                <p style="text-align: right;direction: rtl;">عزيزنا الشريك, {{ $data['user']->company->t('name') }}</p>--}}
                <p style="text-align: right;direction: rtl;">يوجد عرض سعر جديد بانتظار الموافقة برقم مرجعي : {{ $data['reference_id'] }}</p>
...
{{--                <p>Dear partner, {{ $data['user']->company->t('name') }}</p>--}}
                <p>There is a new quotation awaiting approval with a reference number: {{ $data['reference_id'] }}</p>
